...
export const CURATED_TOKEN_LISTS = {
    majors: {
        id: 'majors',
        name: 'Crypto',
...
} as const satisfies Record<string, CuratedTokenList>;

// Order determines tab order in UI
export const CURATED_LIST_ORDER: CuratedTokenListId[] = [
    'majors',
    'lsts',
    'currencies',
    'rwas',
...
export type CuratedTokenListId = keyof typeof CURATED_TOKEN_LISTS;
...
export function normalizeCuratedTokenListId(raw: string): CuratedTokenListId | null {
    ...
    if (trimmed === 'stables') return 'currencies';
    if (trimmed === 'xstocks') return 'etfs';
    return isCuratedTokenListId(trimmed) ? trimmed : null;
}
